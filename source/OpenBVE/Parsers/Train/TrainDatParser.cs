	...
	internal static partial class TrainDatParser {
		...
		internal static void ParseTrainData(string FileName, System.Text.Encoding Encoding, TrainManager.Train Train) {
					...
					case "#motor_b2":
						{
							...
							while (i < Lines.Length && !Lines[i].StartsWith("#", StringComparison.Ordinal)) {
								...
								if (n >= u) {
									Array.Resize<MotorSoundTableEntry>(ref Tables[msi].Entries, 2 * u);
									for (int j = u; j < 2 * u; j++) {
							...
							if (n != 0)
							{
								 ...
								 */
								Array.Resize<MotorSoundTableEntry>(ref Tables[msi].Entries, n);
							}
							i--;
						} break;
				}
			...
			for (int i = 0; i < Cars; i++) {
				...
				Train.Cars[i].CarBrake.airCompressor = new Compressor(5000.0, Train.Cars[i].CarBrake.mainReservoir, Train.Cars[i], Program.Sounds);
