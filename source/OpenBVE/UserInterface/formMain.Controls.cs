...
namespace OpenBve {
	internal partial class formMain : Form {
		...
		private void pictureboxJoysticks_Paint(object sender, PaintEventArgs e)
		{
			...
			if (this.Tag == null & listviewControls.SelectedIndices.Count == 1) {
				int j = listviewControls.SelectedIndices[0];
				if (Interface.CurrentControls[j].Method == Interface.ControlMethod.Joystick && Program.Joysticks.AttachedJoysticks.ContainsKey(Interface.CurrentControls[j].Device)) {
